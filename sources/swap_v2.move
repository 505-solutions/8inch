module swap_addr_v2::swap_v2 {
    ...
    // public entry fun initialize_swap_ledger<SrcCoinType>(module_owner: &signer) {
    ...
    //     vector::append(&mut seed, b"swap_seed_v2");
    ...
    public entry fun initialize_swap_ledger<SrcCoinType>(owner: &signer) {
        let seed = b"swap_seed_v2_1";
        let module_owner_addr = signer::address_of(owner);
        let swap_addr = account::create_resource_address(
            &module_owner_addr,
            seed
        );
