...
describe('Resolving example', () => {
    ...
    beforeAll(async () => {
        ...
        // Add balance checks
        console.log('Source Chain Resolver ETH Balance:', await src.provider.getBalance(await srcChainResolver.getAddress()))
        console.log('Destination Chain Resolver ETH Balance:', await dst.provider.getBalance(await dstChainResolver.getAddress()))
    ...
    describe('Fill', () => {
        it('should swap Ethereum USDC -> Bsc USDC. Single fill only', async () => {
            ...
            console.log('Before transaction - DST Resolver ETH Balance:', await dst.provider.getBalance(await dstChainResolver.getAddress()))
            ...
            const {txHash: orderFillHash, blockHash: srcDeployBlock} = await srcChainResolver.send(
                resolverContract.deploySrc(
                    order.toSrcImmutables(srcChainId, new Address(this.srcAddress), amount, hashLock)
                )
            )
...
async function getProvider(cnf: ChainConfig): Promise<{node?: CreateServerReturnType; provider: JsonRpcProvider}> {
    ...
    const node = createServer({
        instance: anvil({
            forkUrl: cnf.url,
            chainId: cnf.chainId,
            forkBlockNumber: cnf.blockNumber
        }),
        limit: 1
    })
    await node.start()
...
class Immutables {
    constructor(
        ...
        public safetyDeposit: bigint,
        public timelocks: Sdk.TimeLocks
    ) {}
    ...
    withDeployedAt(deployedAt: bigint): Immutables {
        return this
    }
}
})